 ...
 *
 */
class TWCClientHandler extends SimpleChannelInboundHandler<TWCMessage> {
	private TWCClient twcClient;

	public TWCClientHandler(TWCClient twcClient) {
	...
	public void channelRead0(ChannelHandlerContext ctx, TWCMessage twcMessage) {
		try {
			TWCConnect twcConnect = this.twcClient.twcConnectMap.get(ctx.channel().attr(Constant.TWC_CLIENT_CONNECT_ID).get());

			// 业务线程池处理
			twcConnect.twcClient.bizThreadPool.execute(new TWCClientBizTask(ctx, twcMessage, twcConnect));
		} catch (Exception e) {
		...
		try {
			if (evt instanceof IdleStateEvent) {
				...
				if (event.state().equals(IdleState.READER_IDLE)) {
					...
					if (this.twcClient.twcClientConfig.getIsDebug()) {
					...
					twcRequest.setMsgId(CommonLIB.getMsgId(this.twcClient.msgId));
					twcRequest.setMethodId(Constant.METHODID_HEARTBEAT);
					// 写入全局netty处理缓存，用于回调处理
					this.twcClient.methodSyncCache.put(twcRequest.getMsgId(), twcRequest.getMethodId());
	...
	private String getLogPre(ChannelHandlerContext ctx) {
		return this.twcClient.serviceName + "-" + ctx.channel().attr(Constant.TWC_CLIENT_CONNECT_ID).get();
	}
}